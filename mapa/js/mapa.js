...
function carregarPontos() {
    $.getJSON('http://localhost/Java/painelCadastros/server/ListarCoordenadas.php', function(pontos) {
        ...
        $.each(pontos, function(index, ponto) {
            ...
            var contentString = '<div id="mostrarSlides' + ponto.id + '"></div>' +
                '<div class="col-md-13">' +
                ...
                '<p>Tel :  ' + ponto.fixo + '</p>' +
                '<p><img src="img/Images-icon.png" style="display: inline; margin-right: 2px;" class="btn btn-info btn-lg" data-toggle="modal" onclick="abrirModalSlide(' + ponto.id + ')" data-target="#abrirSlide">' +
                '<img src="img/Button-Info-icon.png" class="btn btn-info btn-lg" data-toggle="modal" onclick="carregarmodal(' + ponto.id + ')" data-target="#abrirInfo"></p>' +
                '</div>' +
                '</div>' +
                '</div>' +
                '</div>';
        ...
        var mc = new MarkerClusterer(map, markers, {
            imagePath: 'http://localhost/Java/mapa/img/marcador'
        });
...
function carregarmodal(id) {
    ...
    $.getJSON('http://localhost/Java/painelCadastros/server/listarDados.php?search=', {
            valor: id,
            ajax: 'listarDados'
        },
        function(j) {
            ...
            conteudo = '<div class="col-md-13">' +
                ...
                '<input type="text" value="Rua: ' + rua + '" class="form-control" disabled="">' +
                '</div> ' +
                '<div class="col-xs-3">' +
    ...
    $.getJSON('http://localhost/Java/painelCadastros/server/listarDados.php?search=', {
        ...
        function(j) {
            ...
            conteudoDiv = '<div id="slideshow">' +
                ...
                '<ul class="slides">' +
                nome_foto +
                //'<li><img src="http://localhost/painelCadastros/server/Fotos/'+ j[1] + '"></li>'+
                '</ul>' +
                '</div>' +
                '</div>';
            $('#mostrarSlides' + id).html(conteudoDiv);
